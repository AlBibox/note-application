    ...
    <main className="min-h-screen bg-background font-sans text-foreground">
      ...
      <div className="flex min-h-screen">
        <aside className={`${sidebarOpen ? 'flex' : 'hidden'} w-64 shrink-0 flex-col border-r border-border bg-sidebar px-4 py-5 md:flex`}>
          ...
          {view !== 'Archive' && view !== 'Trash' && <button onClick={() => { setSettingsOpen(false); void createNote() }} className="mt-8 flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2.5 text-sm font-medium text-primary-foreground shadow-sm transition hover:opacity-90"><Plus className="size-4" /> New note</button>}
          <nav className="mt-7 flex flex-col gap-1" aria-label="Main navigation">{navItems.map(({ label, icon: Icon }) => <button key={label} onClick={() => { setSettingsOpen(false); setView(label); setFolderFilter(null); setQuery(''); setActiveNote(0) }} className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm ${view === label && !folderFilter ? 'bg-accent font-medium text-foreground' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`}><span className="flex items-center gap-3"><Icon className="size-4" />{label}</span></button>)}</nav>
          <div className="mt-8 border-t border-border pt-6"><div className="flex items-center justify-between px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground"><span>Folders</span><button aria-label="Add folder" onClick={() => showNotice('Folder creation is ready for the next step')}><Plus className="size-3.5" /></button></div>{['Work', 'Personal', 'Journal'].map((folder) => <button key={folder} onClick={() => { setSettingsOpen(false); setFolderFilter(folder); setView('All notes'); setQuery(''); setActiveNote(0) }} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm ${folderFilter === folder ? 'bg-accent font-medium text-foreground' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`}><Folder className="size-4" />{folder}</button>)}</div>
          <div className="mt-auto flex flex-col gap-1"><button aria-expanded={settingsOpen} onClick={() => setSettingsOpen((open) => !open)} className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><Settings className="size-4" />Settings</button>{settingsOpen && <button onClick={() => { void signOut({ fetchOptions: { onSuccess: () => window.location.assign('/register') } }) }} className="ml-3 rounded-lg px-3 py-2 text-left text-sm text-muted-foreground hover:bg-accent hover:text-foreground">Log out</button>}<div className="mt-3 flex items-center gap-3 border-t border-border px-2 pt-4"><div className="flex size-8 items-center justify-center rounded-full bg-accent text-xs font-semibold">{session.user.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join('').toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{session.user.name}</p><p className="truncate text-xs text-muted-foreground">Personal workspace</p></div><ChevronDown className="ml-auto size-4 text-muted-foreground" /></div></div>
        </aside>
